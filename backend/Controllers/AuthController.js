...
const signup = async (req, res) => {
    try {
        ...
        const userModel = new UserModel({ name, email, password, isAdmin });
        userModel.password = await bcrypt.hash(password, 10);
        await userModel.save();
        res.status(201)
            .json({
...
const createSampleAdminUser = async () => {
    try {
        ...
        console.error('Error creating sample admin user:', err);
    }
};
testAdminUser = createSampleAdminUser();
